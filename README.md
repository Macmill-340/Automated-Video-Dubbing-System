...
(task=translate → English segments) → edge-tts per-segment MP3s →
...
uv run dub-video "<YouTube URL>" [--out dubbed.mp4] [--workdir work/my-dub] `
  [--model-size small] [--voice en-US-AriaNeural] [--rate +0%] [--concurrency 4]
```

- `--model-size`: `tiny` (fastest) → `large-v3` (best). `small` is the sweet spot.
- `--voice`: any `edge-tts --list-voices` name, e.g. `en-US-ChristopherNeural`.
