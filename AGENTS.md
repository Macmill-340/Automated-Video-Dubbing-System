...
- Long media runs (eval videos in `work/eval_30m/` + `work/eval_2h/`): intermediates stay in the workdir (gitignored), video stream copied (`-c:v copy`), timing logged per stage to `timing.json`.
